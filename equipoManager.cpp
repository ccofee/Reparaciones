...
void EquipoManager::cargarCamposEditables(Equipo &e) {
   string cuit;

   do {
      cuit = cargarTexto("CUIT del cliente propietario: ", 14);
      if (!clienteExiste(cuit)) cout << " > El cliente no existe o esta eliminado." << endl;
   } while (!clienteExiste(cuit));
   e.setCuit(cuit);

   e.setDescripcion(cargarTexto("Descripcion: ", 49));
   e.setMarca(cargarTexto("Marca: ", 29));
   ...
   e.setFechaIngreso(cargarFecha("Fecha de ingreso:"));
}
...
void EquipoManager::alta() {
   ...
   do {
      nro = cargarEntero("Numero de equipo: ");
      if (nro <= 0) cout << " > Debe ser mayor a 0." << endl;
      else if (!nroEquipoUnico(nro)) cout << " > El numero de equipo ya existe." << endl;
   } while (nro <= 0 || !nroEquipoUnico(nro));
   e.setNroEquipo(nro);
